...
> = props => {
  ...
  const [contantCopy, setContantCopy] = useState(content)
  let lineH = 0 // 当行的最大高度
  ...
  const calcEllipse = async () => {
    ...
    if (refe.height <= maxHeight) {
      ...
      if (direction === 'middle') {
        ...
        setLeading(tailorContent(0, rowNum * (Number(rows) + 0.5), 'end'))
      ...
      // setTimeout(() => {
      //   verifyEllipsis()
      //   console.log(ellipsis.current.leading, 'verifyEllipsis')
      // }, 100)
    }
  }
  ...
  return (
    <>
      ...
      >
        <div>
          {!exceeded ? (
            <div className='adm-ellipsis-wordbreak'>{content}</div>
          ) : null}
          {exceeded && !expanded ? (
            <>
              <div className='adm-ellipsis-wordbreak'>
                {leading}
                {leading && symbol}
                {expandText ? (
                  <span
                    className='adm-ellipsis-text'
                    onClick={e => {
                      e.stopPropagation()
                      clickHandle(1)
                    }}
